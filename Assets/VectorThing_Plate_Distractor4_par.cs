...
public class VectorThing_Plate_Distractor4_par : MonoBehaviour {
		...
		if(didChange==true){
			...
			for(int i=0; i<charges.Length; i++){
				...
				for(int j = 0; j < chargeStepsInt; j++){
					...
					Vector3 tmp = new Vector3(((kq/(r*r))/12),((0*kq/(r*r))*((transform.position.y - chargeCenters[j].y)/r)),((0*kq/(r*r))*((transform.position.z - chargeCenters[j].z)/r)));
					realVector.x += tmp.x;
					realVector.y += tmp.y;
					realVector.z += tmp.z;
				}
