...
public class CardboardEye : MonoBehaviour {
  ...
  private void FixProjection(ref Matrix4x4 proj) {
    ...
    proj[0, 0] *= GetComponent<Camera>().rect.height / GetComponent<Camera>().rect.width / 2;
  ...
  public void UpdateStereoValues() {
    ...
    GetComponent<Camera>().targetTexture = monoCamera.targetTexture ?? Cardboard.SDK.StereoScreen;
    if (GetComponent<Camera>().targetTexture == null) {
      ...
      GetComponent<Camera>().rect = FixViewport(GetComponent<Camera>().rect);
    }
  }
  ...
  private void SetupStereo() {
    ...
    if (updatePosition) {
      // Set view transform.
      float proj11 = GetComponent<Camera>().projectionMatrix[1, 1];
    ...
    if (Cardboard.SDK.DistortionCorrection == Cardboard.DistortionCorrectionMethod.None) {
      ...
      var fixProj = GetComponent<Camera>().cameraToWorldMatrix *
                    Matrix4x4.Inverse(GetComponent<Camera>().projectionMatrix) *
                    realProj;
      ...
      Shader.SetGlobalMatrix("_RealProjection ", GetComponent<Camera>().projectionMatrix);
      Shader.SetGlobalMatrix("_FixProjection ", GetComponent<Camera>().cameraToWorldMatrix);
    }
    Shader.SetGlobalVector("_Projection", projvec);
    Shader.SetGlobalVector("_Unprojection", unprojvec);
    Shader.SetGlobalFloat("_NearClip", GetComponent<Camera>().nearClipPlane);
  }
