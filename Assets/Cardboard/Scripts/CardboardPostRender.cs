...
public class CardboardPostRender : MonoBehaviour {
  ...
  void Reset() {
    ...
    GetComponent<Camera>().backgroundColor = Color.magenta;  // Should be noticeable if the clear flags change.
  ...
  void OnPreCull() {
    ...
    GetComponent<Camera>().orthographicSize = 0.5f * Mathf.Max(1, aspectComparison);
  }
#endif
