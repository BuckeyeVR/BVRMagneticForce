...
public class VectorThing_Distractor2_scaling : MonoBehaviour {
		...
		if(didChange==true){
			...
			for(int i=0; i<charges.Length; i++){
				...
				for(int j = 0; j < chargeStepsInt; j++){
					...
					float rorig = Mathf.Sqrt((transform.position.x*transform.position.x)+(transform.position.z*transform.position.z));
					...
					Vector3 tmp = new Vector3((rorig*rorig*(kq/(r*r))*((transform.position.x - chargeCenters[j].x)/r)),(rorig*rorig*(kq/(r*r))*((transform.position.y - chargeCenters[j].y)/r)),(rorig*rorig*(kq/(r*r))*((transform.position.z - chargeCenters[j].z)/r)));
					realVector.x += tmp.x;
					realVector.y += tmp.y;
					realVector.z += tmp.z;
				}
