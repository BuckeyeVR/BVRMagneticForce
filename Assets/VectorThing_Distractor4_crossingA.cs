...
public class VectorThing_Distractor4_crossingA : MonoBehaviour {
		...
		if(didChange==true){
			...
			for(int i=0; i<charges.Length; i++){
				...
				for(int j = 0; j < chargeStepsInt; j++){
					...
					Vector3 tmp = new Vector3(((kq/(r*r))*((transform.position.x - chargeCenters[j].x)/r)),((kq/(r*r))*(((transform.position.y - chargeCenters[j].y)/r)-transform.position.y)),((kq/(r*r))*((transform.position.z - chargeCenters[j].z)/r)));
					realVector.x += tmp.x;
					realVector.y += tmp.y;
					realVector.z += tmp.z;
				}
